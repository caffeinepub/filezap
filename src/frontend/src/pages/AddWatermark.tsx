...
function WatermarkOptions({
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 flex flex-col gap-4">
      <div>
        <Label htmlFor="wm-text" className="mb-1.5 block text-sm">
          Watermark Text
        </Label>
        <Input
          id="wm-text"
      ...
      <div>
        <Label className="mb-1.5 block text-sm">Opacity: {opacity}%</Label>
        <Slider
          min={5}
          max={80}
          step={5}
...
export default function AddWatermark() {
  const textRef = useRef("CONFIDENTIAL");
  const opacityRef = useRef(30);
  ...
  return (
    <ToolPageLayout
      ...
      faq={faq}
      optionsPanel={
        <WatermarkOptions textRef={textRef} opacityRef={opacityRef} />
      }
    />
  );
}
